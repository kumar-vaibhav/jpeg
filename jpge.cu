...
/*
template<class T> static void RGB_to_YCC(image *img, const T *src, int width, int y, int height)
{
    //printf("\n%c\t%c\t%c\n", src[0].r,src[0].g,src[0].b);
    for (int x = 0; x < width; x++) {
        const int r = src[x].r, g = src[x].g, b = src[x].b;
        img[0].m_pixels[y*img[0].m_x + x] = ( (0.299     * r) + (0.587     * g) + (0.114     * b)-128.0);
        img[1].m_pixels[y*img[1].m_x + x] = (-(0.168736  * r) - (0.331264  * g) + (0.5       * b));
        img[2].m_pixels[y*img[2].m_x + x] = ( (0.5       * r) - (0.418688  * g) - (0.081312  * b));
    }
}*/
 
//template<class T>
__global__ void RGB_to_YCC(float *img1, float *img2, float *img3,const rgb *src, int width, int y, int height)
{
    // printf("In\n");
    int x = blockIdx.x * blockDim.x + threadIdx.x;  
    if(x >= width*height) return;  
    int r = src[x].r, g = src[x].g, b = src[x].b;
    img1[x] = ( (0.299     * r) + (0.587     * g) + (0.114     * b)-128.0);
    img2[x] = (-(0.168736  * r) - (0.331264  * g) + (0.5       * b));
    img3[x] = ( (0.5       * r) - (0.418688  * g) - (0.081312  * b));
}
...
void jpeg_encoder::load_mcu_YCC(const uint8 *pSrc, int width, int bpp, int y, int height)
{
    //RGB_to_YCC(m_image, reinterpret_cast<const rgb *>(pSrc), width, y, height);
    uint8* pSrc_d  ; cudaMalloc(&pSrc_d, sizeof(uint8)*width*3*height);
    float* img1_d  ; cudaMalloc(&img1_d  , sizeof(float)*width*height);
    float* img2_d  ; cudaMalloc(&img2_d  , sizeof(float)*width*height);
    float* img3_d  ; cudaMalloc(&img3_d  , sizeof(float)*width*height);
    float* r1 = (float*)malloc(sizeof(float)*width*height);
    float* r2 = (float*)malloc(sizeof(float)*width*height);
    float* r3 = (float*)malloc(sizeof(float)*width*height);   


    cudaMemcpy(pSrc_d, pSrc, sizeof(uint8)*width*height*3, cudaMemcpyHostToDevice);

    //printf("**** %d\t%d ****",width, height);

    RGB_to_YCC<<<(width*height*3)/1024,1024>>>(img1_d, img2_d, img3_d, reinterpret_cast<const rgb *>(pSrc_d), width, y, height);

    cudaMemcpy(m_image[0].m_pixels, img1_d, sizeof(float)*width*height, cudaMemcpyDeviceToHost);
    cudaMemcpy(m_image[1].m_pixels, img2_d, sizeof(float)*width*height, cudaMemcpyDeviceToHost);
    cudaMemcpy(m_image[2].m_pixels, img3_d, sizeof(float)*width*height, cudaMemcpyDeviceToHost);
...
bool jpeg_encoder::read_image(const uint8 *image_data, int width, int height, int bpp)
{
    ...
    //for (int y = 0; y < height; y++) {
        load_mcu_YCC(image_data, width, bpp, 3, height);
    //}

    printf ("YCbCr time - %f s\n", float( clock () - begin_time ) /  CLOCKS_PER_SEC);
