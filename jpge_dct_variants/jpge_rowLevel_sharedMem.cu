...
template <class T>
static void RGB_to_YCC(image *img, const T *src, int width, int y)
{
...
template <class T>
static void RGB_to_Y(image &img, const T *pSrc, int width, int y)
{
    for (int x = 0; x < width; x++)
    {
...
void image::load_all_blocks(dct_t *pDst, int wX, int wY)
{
    for (int i = 0; i < wY; i++)
        for (int j = 0; j < wX; j++)
            pDst[i * wX + j] = get_px(j, i);
}
...
inline dct_t image::blend_dual(int x, int y, image &luma)
{
    ...
    return (get_px(x, y) * a + get_px(x + 1, y) * b) / (a + b);
}
...
inline dct_t image::blend_quad(int x, int y, image &luma)
{
    ...
    return (get_px(x, y) * a + get_px(x + 1, y) * b + get_px(x, y + 1) * c + get_px(x + 1, y + 1) * d) / (a + b + c + d);
}
...
bool jpeg_encoder::compress_image()
{
    ...
    for (int c = 0; c < m_num_components; c++)
    {
        ...
        blocks_h[c] = (dct_t *)malloc(sizeof(dct_t) * wX * wY);
    ...
    for (int c = 0; c < m_num_components; c++)
    {
        ...
        cudaMemcpy(blocks_d[c], blocks_h[c], sizeof(dct_t) * wX * wY, cudaMemcpyHostToDevice);
        cudaMemcpy(quant_d[c], quant_h, sizeof(int32) * 64, cudaMemcpyHostToDevice);
        cudaMemcpy(zigzag_d[c], s_zag, sizeof(uint8) * 64, cudaMemcpyHostToDevice);
        ...
        dct<<<blockNum, thrPerBlock, sizeof(dct_t) * memSize>>>(blocks_d[c], out_d[c], quant_d[c], zigzag_d[c], wX / 8, wY / 8);
        // printf("Kernel Exit\n");
        cudaMemcpy(m_image[c].get_dctq(0, 0), out_d[c], sizeof(dctq_t) * wX * wY, cudaMemcpyDeviceToHost);
    }
