...
__global__ void dct(dct_t *data, dctq_t *out, int32 *quant, uint8 *s_zag, int tX, int tY)
{
    ...
    data_ptr = &data[(y * tX + x / 8) * 64 + 8 * (x % 8)];
    ...
    {
        dct_t j = data[(y * tX + x / 8) * 64 + s_zag[i]];
        int32 q = quant[i];
        dctq_t res;
        if (j < 0)
        {
...
bool jpeg_encoder::compress_image()
{
    ...
    for (int c = 0; c < m_num_components; c++)
    {
        ...
        // printf("Kernel Call\n");
        dct<<<blockNum, thrPerBlock>>>(blocks_d, out_d, quant_d, zigzag_d, wX / 8, wY / 8);
        // printf("Kernel Exit\n");
